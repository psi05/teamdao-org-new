		...
		<>
			...
			{(log?.type === 'ai' ||
				...
				<>
					...
					<div
						className={`w-72 flex-grow text-center xl:w-44 ${
							log?.txType === 'sell' ? 'text-[#dd2d44]' : 'text-[#54acee]'
						}`}
					>
						<span>{utils.formatTokenSymbol(log.tokenIn?.symbol)} </span>
						<span>{`$${utils.formatNumber(
							utils.toEther(log.amountsIn, log.tokenIn.decimals) *
								parseFloat(log.tokenIn?.priceUSD ?? '1'),
							true
						)} `}</span>
						<span>{`[${utils.formatNumber(
							utils.toEther(log.amountsIn, log.tokenIn.decimals)
					...
					>
						...
						<span>{`$${utils.formatNumber(
							utils.toEther(log.amountsOut, log.tokenOut.decimals) *
								parseFloat(log.tokenOut?.priceUSD ?? '1'),
							true
						)} `}</span>
						<span>{`[${utils.formatNumber(
							utils.toEther(log.amountsOut, log.tokenOut.decimals)
				...
				<>
					...
					<div className='w-52 flex-grow text-center'>
						<p className='flex items-center justify-center gap-1'>
							{emoji(
								`🔫 Sniped ${utils.formatTokenSymbol(
									log.txType === 'buy'
										? `${log.tokenIn.symbol} 🟦`
										: `${log.tokenOut.symbol} 🟥`
								)}`
